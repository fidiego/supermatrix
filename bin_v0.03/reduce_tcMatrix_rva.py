...
windowSize = 0                      # -W: window size across which co-occurrences will be used. If 0, matrix's max window size will be used
...
the_tcMatrix.importSummedMatrix(1, windowSize, 1, 0)
the_tcMatrix.normalizeCoocMatrix(normalizationMethod)

# run and output the svd
the_rvaModel = rva.rvaModel()
the_rvaModel.initializeModel(matrixDirectory, outputDirectory, the_tcMatrix.coocMatrix, the_tcMatrix.subTargetIndexDict, the_tcMatrix.subContextIndexDict, randomSeed, randomVectorLength, randomVectorMean, randomVectorSD, normalizationMethod)
