...
class svdModel:
    ...
    ############################################################################################
    def initializeModel(self, matrixDirectory, svdDirectory, theMatrix, rowIndexDict, columnIndexDict):
    
        self.matrixDirectory = matrixDirectory
        self.svdDirectory = svdDirectory

        self.outputPath = self.matrixDirectory + self.svdDirectory
    
        if os.path.isdir(self.outputPath):
            print
            print "     Warning: The SVD model directory %s already exists in the %s matrix directory." % (matrixDirectory, self.svdDirectory)
            userInput = raw_input("     Do you want to erase and overwrite this SVD model directory? (y/n) -->")
    ...
    def computeSVD(self):
        print "...Computing SVD"
        self.rowSingularVectors, self.singularValues, self.columnSingularVectors = linalg.svd(self.theMatrix) 
    ...
    def outputModelInfo(self):
        ...
        outputFilehandle = open(self.outputPath+"/"+"singular_values.txt", "w")
        for i in range(len(self.singularValues)):
            outputFilehandle.write("%s\n" % self.singularValues[i])
        outputFilehandle.close()
    ...
    ############################################################################################        
    def importModelInfo(self, svdDirectory):
        self.svdDirectory = svdDirectory
        
        # Get svd model parameters from text file
        infoFilehandle = open(svdDirectory+"model_info.txt")
    ...
    def importRowInfo(self, rowInclusionFile, rowExclusionFile):
        ...
        if rowInclusionFile:
            ...
            targetInfoFilehandle = open(self.svdDirectory+"row_info.txt")
            rowCounter = 0
    ...
    def importColumnInfo(self):
        ...
        columnInfoFilehandle = open(self.svdDirectory+"column_info.txt")
    ...
    def importRowSingularVectors(self, numDimensions):
        ...
        rowSingularVectorsFilehandle = open(self.svdDirectory+"row_singular_vectors.txt")
